	...
	public function admin_post_thumbnail( $content, $post_id = null ) {
		...
		if ( strpos( $thumbnail_id, 'ngg-' ) === false) {
			...
			if (version_compare( $wp_version, '3.5', '>=' ) && $thumbnail_id <= 0) {
				...
				$set_thumbnail_link = '<p class="hide-if-no-js"><a title="' . esc_attr__( 'Set NextGEN featured image', 'nggallery' ) . '" href="' . \Imagely\NGG\Util\Router::esc_url( $iframe_src ) . '" id="set-ngg-post-thumbnail" class="thickbox">%s</a></p>';
